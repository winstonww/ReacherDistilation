...
class Dataset(object):
    ...
    def dump(self):
        try:
            ...
            print(error)
            pass
        assert len(self.data_in_memory) <= MAX_CAPACITY, " length of data exceeds max" 
    ...
    def reset_training_data(self ):
        ...
        rand_pages = self.dstore.rand_pages(15)
    ...
    # training batch for Back-propagation through time
    def training_batches(self):

        if ( len(self.data_in_memory) == 0 ) or not self.training_data:
            self.reset_training_data()

        episodes = [ random.choice(self.training_data) for _ in range(LSTM_BATCH_SIZE) ] 

        for i in range(0, EPISODE_STEPS-STEPS_UNROLLED, 1):

            ob_batch_array            = self.serialize("ob", episodes, i , i+STEPS_UNROLLED)
            t_pdflat_batch_array      = self.serialize('t', episodes, i, i+STEPS_UNROLLED)
            prev_pdflat_batch_array   = self.serialize("prev", episodes, i, i+STEPS_UNROLLED)
            prev_rew_batch_array   = self.serialize("prew", episodes, i, i+STEPS_UNROLLED)
